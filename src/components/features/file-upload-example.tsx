...
export function FileUploadExample() {
  ...
  return (
    <div className='space-y-4'>
      ...
      {uploading && <div className='text-sm text-gray-600'>上传中...</div>}

      {error && <div className='text-sm text-red-600'>错误: {error}</div>}

      {uploadedUrl && (
        <div className='space-y-2'>
          <div className='text-sm text-green-600'>上传成功！</div>
          <div className='text-sm break-all text-gray-600'>文件 URL: {uploadedUrl}</div>
          {uploadedUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i) && (
            <img src={uploadedUrl} alt='Uploaded' className='mt-2 max-w-md rounded-lg border' />
          )}
        </div>
      )}
    </div>
