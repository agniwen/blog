...
function Xt(e, r, t, n2) {
  if (typeof t == "string") {
    ...
    if (D(e.base, n2)) {
      let i = m(e, n2.i);
      return (
        O(e.base, r.i), s && a !== a ? Ur(e, r.i, t, i) : Ne(e, r.i, s ? t : '"' + t + '"', i), ""
      );
    }
    return (s ? t : '"' + t + '"') + ":" + f(e, n2);
  }
...
const TSS_SERVER_FUNCTION_FACTORY = /* @__PURE__ */ Symbol.for("TSS_SERVER_FUNCTION_FACTORY");
      ...
      return Object.assign(
        ...
        },
        {
          ...
          __executeServer: async (opts) => {
            ...
            const result = await executeMiddleware$1(resolvedMiddleware, "server", ctx).then(
              (d) => ({
...
const INITIAL_SCRIPTS = [mn(SCOPE_ID), minifiedTsrBootStrapScript];
class ScriptBuffer {
  constructor(router) {
...
function setCookie$1(event, name, value, options) {
  ...
  for (const cookie of currentCookies) {
    if (_getDistinctCookieKey(cookie.split("=")?.[0], parseSetCookie(cookie)) === newCookieKey)
      continue;
...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-B_Zd9wPY.mjs");
...
const manifest = {
  "552b33777e12c1e63bf67a573af18ee971800506c1a4bd6a1a586b09541c55b0": {
    functionName: "getCommentsServerFn_createServerFn_handler",
    importer: () => import("./assets/comments-DEkDgrsx.mjs"),
  },
  "4deedae7517058e5c3d7a96e9cc471958a98cbca0cf278ecb344624071b87c52": {
    functionName: "createCommentServerFn_createServerFn_handler",
    importer: () => import("./assets/comments-DEkDgrsx.mjs"),
  },
  bb30a4188a05b4facb3163937ab48afc49025a8af1bf16b8b7fbafe07d39d1ed: {
    functionName: "getPublishedPostsServerFn_createServerFn_handler",
    importer: () => import("./assets/posts-B8hGmarm.mjs"),
  },
  "11f6f2ea6dc0df5f8dca10bda1841334d6eaf9067b68c92f9e4104633e08aff5": {
    functionName: "getPublishedPostServerFn_createServerFn_handler",
    importer: () => import("./assets/posts-B8hGmarm.mjs"),
  },
  d34b813c0f18d135c36bb2d9e67c184fdf92f0f8baa83e302195da0bfaaaa07b: {
    functionName: "getSessionServerFn_createServerFn_handler",
    importer: () => import("./assets/auth-wPqyp4wd.mjs"),
  },
};
      ...
      let serializeResult = function (res2) {
        ...
        if (res2 !== void 0) {
          ...
          if (rawStreams.size > 0) {
            ...
            const multiplexedStream = createMultiplexedStream(jsonStream, rawStreams);
  ...
  return (async () => {
    ...
    for (const route of Object.values(manifest2.routes)) {
      if (route.preloads) {
        route.preloads = await Promise.all(
          route.preloads.map((url) => Promise.resolve(transformFn({ url, type: "modulepreload" }))),
        );
      }
      if (route.assets) {
    ...
    if (rootRoute) {
      ...
      rootRoute.assets.push(
        buildClientEntryScriptTag(transformedClientEntry, source.injectedHeadScripts),
      );
    }
    return manifest2;
  })();
...
async function loadEntries() {
  const routerEntry = await import("./assets/router-CV8uKWT2.mjs").then((n2) => n2.r);
  ...
  const startRequestResolver = async (request, requestOpts) => {
	...
	basepath: ROUTER_BASEPATH
      ...
      if (SERVER_FN_BASE && url.pathname.startsWith(SERVER_FN_BASE)) {
        ...
        const middlewares2 = flattenedRequestMiddlewares.map((d) => d.options.server);
      ...
      const middlewares = flattenedRequestMiddlewares.map((d) => d.options.server);
      const ctx = await executeMiddleware([...middlewares, requestHandlerMiddleware], {
        request,
        pathname: url.pathname,
        context: createNullProtoObject(requestOpts?.context),
      });
