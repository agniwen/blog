import { c as createAdapterFactory, s as sql, b as createKyselyAdapter } from "./auth-BaGll2Xr.mjs";
import { g } from "./auth-BaGll2Xr.mjs";
import "drizzle-orm";
import "zod";
        ...
        for (const currentRow of rows) {
          ...
          if (!groupedByMainId.has(mainId)) {
            ...
            for (const [joinModel, joinAttr] of Object.entries(joinConfig))
              entry$1[getModelName(joinModel)] = joinAttr.relation === "one-to-one" ? null : [];
          ...
          for (const [joinModel, joinAttr] of Object.entries(joinConfig)) {
            ...
            const hasData =
              ...
              Object.values(joinedObj).some((value) => value !== null && value !== void 0);
            if (isUnique) entry[getModelName(joinModel)] = hasData ? joinedObj : null;
              ...
              if (Array.isArray(entry[joinModelName]) && hasData) {
                ...
                if (joinedId) {
                  if (
                    !entry[joinModelName].some((item) => item[idFieldName] === joinedId) &&
          ...
          if (join)
            for (const [joinModel, joinAttr] of Object.entries(join)) {
              const [_joinModelSchema, joinModelName] = joinModel.includes(".")
                ? joinModel.split(".")
                : [void 0, joinModel];
              query = query.leftJoin(`${joinModel} as join_${joinModelName}`, (join$1) =>
                join$1.onRef(
    ...
    config: {
      ...
      supportsBooleans:
        ...
        !config?.type
          ...
          : true,
      supportsDates:
        config?.type === "sqlite" || config?.type === "mssql" || !config?.type ? false : true,
...
export { createKyselyAdapter, g as getKyselyDatabaseType, kyselyAdapter };
